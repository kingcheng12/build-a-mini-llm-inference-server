...
def drive_until_complete(server_state, params, allocator, sampling_config, vocab, max_steps):
    ...
    while step < max_steps and (server_state["waiting_heap"] or server_state["running"]):
        ...
        plan = schedule_step(
            ...
            server_state["max_running"],
        )

        running = plan["running"]
        newly_admitted = plan["newly_admitted"]

        # PREFILL
        for state in newly_admitted:
            state["last_logits"], state["cache"] = model_prefill(
                state["prompt_token_ids"], params
            )
            running.append(state)

        # track emitted length per request
        pre_len = {
            s["request_id"]: len(s["generated"])
            for s in running
        }

        # init completed entry
        for s in running:
            rid = s["request_id"]
            server_state["completed"].setdefault(rid, {
                "output_ids": [],
                "chunks": []
            })
        ...
        for state in running:
            ...
            old = pre_len.get(rid, 0)

            done = is_sequence_done(state, eos_token_id)
            ...
            for tok in state["generated"][old:]:
                ...
                server_state["completed"][rid]["chunks"].append({
                    "token_id": tok,
                    "finished": done
                })
        ...
        server_state["running"] = [
            s for s in running if s["request_id"] not in finished_ids
        ]
...
def latency_percentiles(latencies, percentiles):
    ...
    for p in percentiles:
        ...
        if lo == hi:
            result[float(p)] = latencies[lo]
        else:
            w = idx - lo
            result[float(p)] = latencies[lo] * (1 - w) + latencies[hi] * w
